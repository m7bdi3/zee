...
export default function Navabarmessages({ conversation }: props) {
    ...
    <Card className="w-full p-2">
      <Link href={`/messages/${conversation.id}`} className="cursor-pointer">
        <div className="flex w-full items-center gap-4">
          <UserAvatar src={receiverProfile.avatar_url!} className="h-8 w-8" />
          <div>
            <div className="flex gap-2 w-full items-center">
              <p className="text-sm">@{receiverProfile.username}</p>
            ...
            <div className="flex-col w-full ">
              <div className="flex gap-2 w-full items-center">
                <p className="dark:text-gray-300">
                  {conversation.latest_message.senderProfile?.id === user?.id
